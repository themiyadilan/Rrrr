...
async function connectToWA() {
    ...
    conn.ev.on('messages.upsert', async (mek) => {
        ...
        // Add button base functionality
        const sendButtonMessage = async (jid, buttons, body, footer) => {
            if (config.BUTTON === "on") {
                const buttonMessage = {
                    text: body,
                    footer: footer,
        ...
        // Example button message usage
        if (isCmd) {
            const buttons = [
                { buttonId: 'button1', buttonText: { displayText: 'Button 1' }, type: 1 },
                { buttonId: 'button2', buttonText: { displayText: 'Button 2' }, type: 1 }
            ];
            const footer = "Choose an option:";
            await sendButtonMessage(from, buttons, "Here are your options:", footer);
        }
        ...
        if (isCmd) {
            const cmd = events.commands.find((cmd) => cmd.name === cmdName); // Make sure to match the command name
    if (cmd) {
